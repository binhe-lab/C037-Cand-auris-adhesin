...
```{r}
mdr.freeR <- read.codeml_mlc("../output/paml/MDR-PF11765-freeR/mlc")
mdr.oneR <- read.codeml_mlc("../output/paml/MDR-PF11765-oneR/mlc")
```
```{r}
mdr.freeR.1 <- drop.tip(mdr.freeR, tip = c("XP_462630.1_Dhansenii", "XP_002770057.1_Dhansenii"))
ggtree(mdr.freeR.1, aes(color = dN_vs_dS), branch.length = 't', ) + 
  geom_tiplab(as_ylab = TRUE) +
  geom_text(aes(x = branch, label = round(dN_vs_dS, 2)), vjust = -.5, size = 2) + 
...
### `CodonFreq = 0`
```{r}
caur.freeR.cf0 <- read.codeml_mlc("../output/paml/B8441-Hil1-8-PF11765-freeR-codonfreq0/mlc")
# several internal branches have 0 inferred syn or nonsyn changes, and the dN/dS ratio estimates doesn't make much sense
#caur.freeR@data <- mutate(caur.freeR@data,
#                          dN_vs_dS = ifelse(N_x_dN == 0 | S_x_dS == 0, NA, dN_vs_dS))
# no longer done as this hides important information. instead, suqish the out of bound values in heatmap
ggtree(caur.freeR.cf0, aes(color = dN_vs_dS), branch.length = 't') + 
  #geom_rootedge(rootedge = 0.1) +
  geom_nodelab(hjust = -0.2, size = 2, color = "black") +
  geom_tiplab(as_ylab = TRUE) +
  geom_text(aes(x = branch, label = round(dN_vs_dS, 2)), vjust = -.5) + 
  scale_color_continuous(name='dN/dS', limits=c(0, 1.5),
                         oob=scales::squish,
                         low='darkgreen', high='red')
```
...
### `CodonFreq = 1`
```{r}
caur.freeR.cf1 <- read.codeml_mlc("../output/paml/B8441-Hil1-8-PF11765-freeR-codonfreq1/mlc")
# several internal branches have 0 inferred syn or nonsyn changes, and the dN/dS ratio estimates doesn't make much sense
ggtree(caur.freeR.cf1, aes(color = dN_vs_dS), branch.length = 't') + 
  #geom_rootedge(rootedge = 0.1) +
  geom_nodelab(hjust = -0.2, size = 2, color = "black") +
  geom_tiplab(as_ylab = TRUE) +
  geom_text(aes(x = branch, label = round(dN_vs_dS, 2)), vjust = -.5) + 
