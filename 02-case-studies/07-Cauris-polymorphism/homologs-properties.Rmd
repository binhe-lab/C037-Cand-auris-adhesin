...
## B8441 representative sequence feature plot
...
p4
ggsave(paste0("output/figure/", gsub("-", "", Sys.Date()), "-B8441-domain-tandem-repeats.png"), width = 8, height = 3.5)
```
...
### S/T frequency
For S/T frequencies, I ran `freak` with 30 aa sliding window and 30 aa step size, that is, I divided each protein into 30 aa bins and calculated the frequency in each window.
```{r}
files <- paste0("output/seq-feature/B8441_", c("S", "T", "ST"), "_freq_50_10_freak.out.gz")
names(files) <- c("S", "T", "S/T")
B8441.ST <- map_dfr(files, read_tsv, col_types = "cid", .id = "Var") %>% 
  # reformat pID
  separate(id, into = c("name", "pID", NA), sep = "_") %>% 
  mutate(pID = paste0(pID, "_B8441_Cauris")) %>% 
  # introduce gID for plotting
  left_join(select(len, pID, gID), by = "pID") %>% 
  mutate(Var = ordered(Var, levels = c("S/T","S","T")),
         gID = ordered(gID, levels = seq.order))
```

### N- and O-glycosylation
To directly assess the extent and distribution of N- and O-glycosylation sites in the representative Hil homologs in _C. auris_, I used the following tools to predict the two types of modifications in the eight Hil  homologs from B8441:
...
```{r read_gff()}
# modified from https://cran.r-project.org/web/packages/rmonad/vignettes/gff-processing.html
...
```
Read glycosylation prediction results
```{r}
B8441.NGlyc <- read_tsv("output/glycosylation/20220624-B8441-Hil-homologs-NetNGlyc-results.txt", col_types = cols()) %>% 
  # reformat pID
  separate(seqname, into = c("name", "pID", NA), sep = "_") %>% 
  mutate(pID = paste0(pID, "_B8441_Cauris")) %>% 
  # introduce gID for plotting
  left_join(select(len, pID, gID), by = "pID")

B8441.OGlyc <- read_gff("output/glycosylation/20220624-B8441-Hil-homologs-NetOGlyc-results.txt") %>% 
  separate(seqid, into = c("name", "pID", NA, NA), sep = "_") %>% 
  mutate(pID = paste0(pID, ".1_B8441_Cauris")) %>% 
  left_join(select(len, pID, gID), by = "pID")
```
...
```{r fig.width=8, fig.height=3.5}
w = 50 # window size
p1 <- ggplot(data = B8441.ST %>% filter(Var == "S/T"), aes(x = gID, y = pos + w/2)) + 
  geom_tile(aes(fill = freq), width = 0.6, height = w, alpha = 0.9)
#p2 <- geom_tile(data = B8441.ST %>% filter(Var == "T"), aes(x = gID, y = pos, fill = freq),
                #width = 0.25, alpha = 0.9, position = position_nudge(x = -0.12))
p3 <- geom_segment(data = B8441.NGlyc %>% filter(result %in% c("+", "++")), 
                   aes(xend = gID, y = ifelse(position < 5, 0, position - 5), yend = position + 5),
                   position = position_nudge(x = 0.35, y = 0), color = "red4", size = 1.5)
#p3 <- geom_point(data = B8441.NGlyc %>% filter(result %in% c("+", "++")), aes(x = gID, y = position),
#                 shape = 3, fill = "white", size = 1.5, #stroke = 0, alpha = 0.8,
#                 position = position_nudge(x = .3))
p4 <- geom_segment(data = B8441.OGlyc %>% filter(attr == "+"), 
                   aes(xend = gID, y = ifelse(start < 2, 0, start - 2), yend = start + 2),
                   position = position_nudge(x = -0.35, y = 0), color = "pink3", size = 1.5)

p1 + p3 + p4 + coord_flip() + theme_cowplot() + 
  #scale_fill_viridis_b("Ser/Thr", option = "A", n.breaks = 10) +
  scale_fill_fermenter("Ser/Thr", direction = -1, palette = "YlGnBu", n.breaks = 10) +
  scale_x_discrete(labels = paste0(len$gID, " (", len$name, ")")) +
  #scale_alpha(guide = "none") +
  labs(y = "Position in sequence", x = NULL, color = "FEATURES") + 
  theme(legend.position = c(0.7,0.4),
        legend.box.margin = margin(6,6,6,6),
        legend.box.background = element_rect(size = 0.5, linetype = "solid", color = 1))
ggsave(paste0("output/figure/", gsub("-", "", Sys.Date()), "-B8441-ST-glycosyl.png"),
       width = 8, height = 3.5, dpi = 300)
```

### β-aggregation sequences
...
ggplot(B8441.ST, aes(x = gID, y = pos)) + geom_tile(aes(fill = freq)) + 
  ...
  ggtitle("Ser/Thr frequency in 50 aa windows")
...
B8441.STprot <- read_tsv("input/Cauris-ST-freq-per-protein.txt.gz", col_types = cols()) %>% 
  ...
  mutate(residue = factor(residue, levels = c("Ser", "Thr")),
         Hil = ID %in% str_split(len$pID, "_", simplify = T)[,1])
```
```{r}
B8441.STprot %>% 
  ggplot(aes(value)) + 
  ...
  geom_segment(data = filter(B8441.STprot, Hil), 
...
```
Testing the difference in Ser frequency between the Hil family and the rest of the proteome
```{r}
B8441.STprot %>% 
  filter(residue == "Ser") %>% 
  t.test(value ~ Hil, data = ., var.equal = TRUE)
```

Testing the difference in Thr frequency between the Hil family and the rest of the proteome
```{r}
B8441.STprot %>% 
  filter(residue == "Thr") %>% 
  t.test(value ~ Hil, data = ., var.equal = TRUE)
