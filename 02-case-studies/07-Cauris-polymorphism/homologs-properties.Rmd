...
B8441 <- read_tsv("output/seq-feature/B8441-homologs-feature-table.tsv", col_types = cols()) %>% 
...
#B8441.tandem.div <- read_tsv("output/seq-feature/B8441-homologs-tandem_sub_div.tsv", col_types = cols()) %>% 
#  left_join(select(len, shortName = name, gID, pID), by = c("name" = "pID")) %>% 
#  select(shortName, gID, pID = name, div)
...
p3 <- p1 + p2 + coord_flip() + theme_cowplot() + 
  scale_color_manual(values = colors) + scale_x_discrete(labels = paste0(len$gID, " (", len$name, ")")) +
...
sps.tree %>%
  ggtree() + 
  geom_tiplab(as_ylab = TRUE, size = 12, face = 3) +
  geom_text(aes(x = branch, label = duplications), vjust = -.5, size = 3, color = "red") +
  geom_text(aes(x = branch, label = losses), vjust = 1.5, size = 3, color = "grey20")
```
