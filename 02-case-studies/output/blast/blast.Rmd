...
refseq0 %>% filter(-log10(e.value) < 10)
fungidb %>% filter(gene_id %in% fdb.list, -log10(e_value) < 10)
gryc %>% filter(id %in% gryc1$id, -log10(min_e_value) < 10)
```
Below is the multiple sequence alignment in which I moved all the sequences with e-value > 1e-10 to the top
