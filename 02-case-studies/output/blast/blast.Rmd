...
ggsave("20200701-XP_028889033-homologs-e-value-by-length.png", width = 3, height = 3)
```
...
```
There are a total of `r length(unique(gryc2refseq$query_id))` queries submitted, among which `r nrow(match.gryc)` are redundant with the refseq hits, which are `r match.gryc`. The rest are new entries. To include them in the gene tree reconstruction, we need to get their gene_id and species names.
...
combined.seq <- c(refseqdb.seq, fungidb.seq[fdb.list], gryc.seq[gryc.list])
