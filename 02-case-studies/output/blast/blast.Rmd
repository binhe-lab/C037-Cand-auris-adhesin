...
I blasted XP_028889033 using NCBI blast against the refseq_protein database. (See README.md for details). The taxonomy distribution is shown below:
![taxonomy of refseq_protein blast](img/20200704-ncbi-blastp-XP_028889033-taxonmy-distribution.png)
```{r refseq_protein_blast}
# load table
refseq <- read_csv("XP_028889033_homologs_refprot_select.csv", col_types = "cddnnnc")
refseq.length <- read_tsv("XP_028889033_homologs_refprot_select_length.txt", col_names = c("accession","length"), col_types = "ci")
...
### _Update_ [HB 2021-01-23] NCBI blastp result
...
refseq.N360 <- read_csv("XP_028889033_homologs_refprot_N360.csv",  col_types = "ccccddcddic", col_names = hitTabColNames, skip = 1) %>% 
...
refseq.N360 %>% filter(accession %in% only.in.N360) %>% 
  select(accession, query.cover, e.value, perc.ident, length, sci.name) %>% 
  arrange(sci.name)
...
I repeated the search with either N360 or N560 aa, with e-value 1e-5 cutoff and low complexity filter on. The first search yielded 104 hits and the second 105. I downloaded the results from the first search and named them with "N360" in their filenames. Below I will compare the list to my previous list (filenames don't contain N560 but the query was the first 560 aa)
...
# new: load table
fungidb.new <- read_tsv("XP_028889033_homologs_fungidb_N360_table.tsv", col_types = "cccddcccccccd")
...
```
Based on the results, I will remove the sequences that are shorter than 500 amino acids. Note that the same 50% query coverage filter was not applied here (this was not even reported in the original blast search from fungidb).
...
cat(fdb.filtered$gene_id, file = "fungidb_filter_list.txt", sep = "\n")
```
70 sequences left
...
1. Note that I should exclude the sequence(s) from _C. auris_ B8441 as the refseq blast results, which I used as the basis of my homolog table, contains B11221. Including this B8441 sequence would make _C. auris_ the only species represented by more than one strain in the homolog list.
1. The remaining five sequences belong to _C. paradoxus_ (4) and _C. tropicalis_ (1)
1. See the "Merge" section for choices to include some additional sequences.
...
Added new sequences from the refseq_prot search.

### FungiDB
The `bash` script below is to make a blast database from the NCBI blast results so that we can map the FungiDB hits to it to identify redundant sequences.
```{bash blast}
# make blast database from the NCBI blast results
mkdir blastdb
makeblastdb -in XP_028889033_homologs_refprot_N360.fasta -parse_seqids -dbtype prot -title XP_028889033_refprot -out blastdb/XP_028889033_refprot
# blastp
blastp -db ./blastdb/XP_028889033_refprot -query XP_028889033_homologs_fungidb_use.fasta -outfmt "6 qseqid sseqid qlen slen pident mismatch score bitscore evalue" -max_target_seqs 1 -num_threads 4 -out XP_028889033_fungidb-refprot-N360-blast.txt
blastp -db ./blastdb/XP_028889033_refprot -query XP_028889033_homologs_gryc.fasta -outfmt "6 qseqid sseqid qlen slen pident mismatch score bitscore evalue" -max_target_seqs 1 -num_threads 4 -out XP_028889033_gryc-refprot-N360-blast.txt
```
...
fdb2refseq <- read_tsv("XP_028889033_fungidb-refprot-N360-blast.txt", col_types = "ccnndnnnn", col_names = blast.header)
...
cat("do we indeed have one entry per query?")
match %>% add_count(query_id) %>% filter(n > 1) %>% arrange(desc(n))
cat("answer is yes")

cat("which proteins are unique to the fungidb hits?")
uniq.fdb <- setdiff(fdb2refseq$query_id, match$query_id)
paste(uniq.fdb, collapse = " ")

cat("were they unqiue because the sequences were not in refseq or because they have low e-values?")
fungidb %>% 
  filter(gene_id %in% uniq.fdb) %>% 
  mutate(refseq_id = fdb.refseqID[gene_id]) %>% 
  select(gene_id, refseq_id, organism, e_value)
```
Note: any rows with "NA" in the "refseq_id" column means the fungidb hit is not included in the refseq_protein database. The rest are in the refseq_protein database but not included in my blast search against that database. Among those, CAGL0L00227g and CLUG_05233 are both "provisional" refseq sequences, meaning that their records have not been submitted to NCBI for final review. Also, among those sequences all but CLUG_05233 have lower than 50% query coverage. All in all, I think I can safely use the refseq hits as the base and add _C. auris_ B9J08_004098, CLUG_05233 and the _C. parapsilosis_ sequences (for some reason this species is not included in refseq). To do so, I'll use the `read.fasta()` function from the `seqinr` package to extract their sequences, and use the `write.fasta()` function to output the combined fasta.
...
refseqdb.seq <- read.fasta("XP_028889033_homologs_refprot_N360.fasta", seqtype = "AA", as.string = TRUE, strip.desc = TRUE)
...
fdb.list <- c("CLUG_05233",  paste("CPAR2",c(600430,806390,806420),sep = "_"))
fdb.species <- c("Clusitaniae", rep("Cparapsilosis",times = 3))
...
write.fasta(combined.seq, names = combined.names, file.out = "XP_028889033_homologs_combine_for_gene_tree.fasta")
```

## Obtain the chromosomal locations for the homologous genes
...
refseq.strain <- unique(with(refseq1, paste(species, strain, sep = " ")))
...
fdb.strain <- c("Clavispora lusitaniae ATCC 42720", "Candida parapsilosis CDC317")
gryc.strain <- unique(gryc1$species)
all.strain <- unique(c(refseq.strain, fdb.strain, gryc.strain))
tmp <- sapply(refseq.strain, function(i){
...
sps.list <- unique(refseq2$species)
sps.list
...
sps.stat <- tibble(species = character(), strain = character(), 
                   assemblyaccession = character(), assemblyname = character(), assemblystatus = character(),
...
for(f in c(ftpreport, ftpfeature)){
  download.file(f, paste0("./assembly-info/",last(str_split(f, "/")[[1]])))
}
```
...
search_chr_loc <- function(gene){
  cmd <- sprintf("zgrep %s ./assembly-info/*feature_table.txt.gz | grep ':gene'", gene)
...
4. Finally, we can combine the Refseq results with the mannually entered GRYC and FungiDB results
```{r}
non.refseq <- read_tsv("XP_028889033_homologs_chr_loc_fungidb_gryc.txt", comment = "#")
...
# chrL; relLoc
refseq4 <- refseq3 %>% 
  rename(gene_id = alias, gene_name = gene_id, chr_name = chrloc, assemblyaccession = assembly) %>% 
  left_join(sps.stat %>% filter(refseq !=  "na") %>% 
              select(species, strain, assemblyaccession, assemblystatus, starts_with("n_"))) %>% 
  ...
  select(source, accession, species, species_id, strain, assemblyaccession, assemblystatus, n_seqs, gene_id, gene_name,
         chraccver, chr_name, chrstart, chrstop, chrL, relLoc)
all.genes <- bind_rows(refseq4, non.refseq)
write_tsv(all.genes, "./XP_028889033_homologs_chr_loc.tsv")
```
...
write.fasta(highE.seq, names = highE.names, 
            file.out = "XP_028889033_homologs_high_Evalue.fasta")
```

When I submitted the `r length(highE.names)` sequences to the [CDD batch search site](https://www.ncbi.nlm.nih.gov/Structure/bwrpsb/bwrpsb.cgi) and downloaded the results. Below is a screenshot of the hits:
