...
```{r merge2}
# export the IDs
tmp.list <- c("B9J08_004098","CLUG_05233",paste("CPAR2",c(600430,806390,806410,806420),sep = "_"))
...
combined.seq <- c(refseqdb.seq, fungidb.seq[tmp.list])
write.fasta(combined.seq, names = getAnnot(combined.seq), file.out = "XP_028889033_homologs_combine.fasta")
```

Next we extract those sequences and add them to the refseq file
```{bash}
# python ../../script/extract_fasta.py XP_028889033_homologs_fungidb_use.fasta fungidb_add2refseq.txt
#cat XP_028889033_homologs_refprot.fasta fungidb_add2refseq.fasta > XP_028889033_homologs_combine.fasta
#rm fungidb_add2refseq.fasta
which python
```
...
refseq1 <- cbind(refseq.orgn, refseq) %>% 
  mutate(organism = word(str_replace_all(organism, "[\\x5B\\x5D]", ""), 1, 2)) %>% # \\x5B\\x5D are Hexadecimal representations of the square brackets
