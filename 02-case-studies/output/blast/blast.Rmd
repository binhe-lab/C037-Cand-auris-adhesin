...
hitTabColNames <- c("description","sci.name","common.name","taxid","max.score",
                    "total.score","query.cover","e.value","perc.iden","acc.len","acc")
...
all.N360 <- refseq.N360 %>% filter(query.cover >= 50, acc.len > 500) %>% pull(accession)
only.in.N360 <- setdiff(all.N360, refseq0$accession)
refseq.N360 %>% filter(accession %in% only.in.N360) %>% 
  select(accession, query.cover, e.value, perc.iden, acc.len, sci.name) %>% 
  arrange(sci.name)
```
...
1. Among the 10 sequences that didn't map, five belong to the _C. auris_ B8441 strain, which is the only strain of _C. auris_ present in the FungiDB, but apparently is not in the refseq_protein database. Another three sequences in B8441 apparently are identical with sequences in the other _C. auris_ strains included in the refseq database.
...
mkdir blastdb
makeblastdb -in XP_028889033_homologs_refprot_N560.fasta -parse_seqids -dbtype prot -title XP_028889033_refprot -out blastdb/XP_028889033_refprot
# blastp
...
fdb2refseq <- read_tsv("XP_028889033_fungidb-refprot-blast.txt", col_types = "ccnndnnnn")
names(fdb2refseq) <- c("query_id","subject_id","query_length","subject_length","perc_identity","mismatch","score","bitscore","e_value")
...
```
Note: any rows with "NA" in the "refseq_id" column means the fungidb hit is not included in the refseq_protein database. The rest are in the refseq_protein database but not included in my blast search against that database. Among those, CAGL0L00227g and CLUG_05233 are both "provisional" refseq sequences, meaning that their records have not been submitted to NCBI for final review. Also, among those sequences all but CLUG_05233 have lower than 50% query coverage. All in all, I think I can safely use the refseq hits as the base and add _C. auris_ B9J08_004098, CLUG_05233 and the _C. paradoxus_ sequences (for some reason this species is not included in refseq). To do so, I'll use the `read.fasta()` function from the `seqinr` package to extract their sequences, and use the `write.fasta()` function to output the combined fasta.
...
refseqdb.seq <- read.fasta("XP_028889033_homologs_refprot_N560.fasta", seqtype = "AA", as.string = TRUE, strip.desc = TRUE)
