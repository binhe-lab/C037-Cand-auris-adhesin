...
ggtree(gene.tree.rec, ladderize = TRUE, branch.length = "none") + 
  ...
  geom_tippoint(aes(color = species_gr)) +
  scale_color_manual(name = "Clades",
                     values = c("MDR clade" = "purple", "C. auris" = "plum2",
...
tr <- tandem %>% 
  ...
  mutate(type = ifelse(period * copyMean < 40, "TR-short (<40 aa)", "TR-long (>40 aa)"),
...
tr.feature <- bind_rows(feature, tr) %>% 
  mutate(type = ordered(type, levels = c("entire protein", "Hyphal_reg_CWP", "SignalP", "GPI-anchor", "TR-long (>40 aa)", "TR-short (<40 aa)")))
...
write_tsv(tr.feature, file = "output/misc/R-feature-table.tsv", col_names = TRUE)
```
...
```{r plot_tandem_repeats, warning=FALSE, fig.height=9, fig.width=10}
# plot
...
plot_grid(p.gtree, p3, ncol = 2, align = 'v', rel_widths = c(1.5,4), scale = c(1.02,1))
ggsave("output/img/20210625-domain-tandem-repeats.png", width = 7, height = 7.5)
```
