...
seqInfo <- seqInfo %>% 
  ...
  mutate(assemblystatus = factor(assemblystatus, levels = c("Complete Genome","Chromosome","Contig","Scaffold"), 
         ...
         species_gr = factor(species_id, labels = 
                               c(rep("MDR clade",4),"C. lusitaniae","D. hansenii",
                                 rep("Candida",5), "S. stipitis", rep("Saccharomycetaceae",6)))) %>% 
...
p <- ggtree(sps.tree.exp, ladderize = FALSE) + xlim(0,16) + scale_y_reverse() +
  ...
  geom_hilight(node = 35, fill = "orange", alpha = 0.25)     + # albicans
  ...
  geom_cladelabel(node = 33,  label = "Candida", offset = off, color = "gold3", barsize = 0.2, 
                  offset.text = 0.3, angle = 270, hjust = .5, extend = 0.3) + # Candida
  geom_cladelabel(node = 35,  label = "albicans", offset = off+1, color = "orange", fontface = 2,
...
sps.color <-  c("MDR clade" = "#7F00FF9F", "C. auris" = "darkorchid",
                "C. lusitaniae" = "plum2", "D. hansenii"  = "gray50",
                "S. stipitis" = "slategray", 
                "Candida" = "orange2",  "Saccharomycetaceae" = "grey20")
ggtree(gene.tree.rec, ladderize = FALSE, branch.length = "none") + 
  scale_y_reverse() +
...
ggsave("output/img/20210624-reconciled-rearranged90-gene-tree-color.png", width = 7.5, height = 7)
```
...
sps.tree %>% 
  ...
  geom_hilight(node = 29, fill = "orange", alpha = 0.25)     + # albicans
...
df1 <- seqInfo %>% 
  ...
  right_join(df) %>% 
  select(species = species_id, Total = n, Pass, FungalRV = fungalRV, SignalP, GPI_Pred)

dt1 <- DT::datatable(df1, 
                     options = list(dom = 't', pageLength = 20, ordering = FALSE, 
                                    autowidth = TRUE,
                                    # https://rstudio.github.io/DT/options.html
                                    columnDefs = list(list(width = "150", targets = 1),
                                                      list(width = '50', className = "dt-center", targets = 2:6))), 
                caption = "Summary of Adhesin Prediction Results")
# the following code is from https://www.r-bloggers.com/2020/05/mimic-excels-conditional-formatting-in-r-2/
...
feature <- feature %>% 
  mutate(tip = ifelse(type == "entire protein", as.character(name), as.character(type)),
         ...
         type = ordered(type, levels = c("entire protein", "Hyphal_reg_CWP", "SignalP", "GPI-anchor")))
...
tr <- tandem %>% 
  ...
  mutate(type = ifelse(period * copyMean < tr.th, "TR-short", "TR-long")) %>% 
...
tr.feature <- bind_rows(feature, tr) %>% 
  mutate(type = ordered(type, levels = c("entire protein", "Hyphal_reg_CWP", "SignalP", "GPI-anchor", "TR-long", "TR-short")))
...
# combine the colors
comb.col <- c(feature.colors, "#a87b00bb", "#ffbb00bb")
```

```{r plot_tandem_repeats, warning=FALSE, fig.width=6, fig.height=7.5}
# plot
p1 <- ggplot(tr.feature, aes(x = name, y = start, xend = name, yend = end)) + 
  geom_segment(aes(color = type, text = tip), size = 2)
...
p3 <- p1 + coord_flip() + theme_classic() + 
  ...
  labs(title = "Domain schematics of Hil family proteins", y = "Position in sequence", x = "Sequences", color = "FEATURES")
p3
...
```{r} 
# combine sequence features with tandem repeats
tr1.feature <- bind_rows(feature, tr1) %>% 
  mutate(type = ordered(type, levels = c("entire protein", "Hyphal_reg_CWP", "SignalP", "GPI-anchor", unique(tr1$type))))
comb.col1 <- c(feature.colors, paste0(tr.col, "CC")) # add 20% transparency to the TR features
write_tsv(tr1.feature, file = "output/misc/R-feature-table.tsv", col_names = TRUE)
```

```{r, warning=FALSE, fig.width = 6, fig.height=7.5}
# plot
p1 <- ggplot(tr1.feature, aes(x = name, y = start, xend = name, yend = end)) + 
  geom_segment(aes(color = type, text = tip), size = 2)
p2 <- geom_segment(data = tandem.div, aes(x = name, xend = name, y = div, yend = div + 1.5), size = 2, color = "white")
p3 <- p1 + coord_flip() + theme_classic() + 
  scale_color_manual(values = comb.col1) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.02))) +
  theme(axis.text.y = element_text(size = 4), axis.title.y = element_blank(),
        axis.line.y = element_blank(), axis.ticks.y = element_blank(),
        axis.line.x = element_blank(),# axis.ticks.x = element_blank(),
...
ggsave("output/img/20210624-species-tree-side.png", p.tree, width = 4, height = 6)
```
...
tr.length %>% 
  ggplot(aes(x = trLen, y = length, color = species_gr)) + 
...
```{r}
tr.length %>% 
  ggplot(aes(x = nonNTDLen, y = tr.content, color = species_gr)) + 
  geom_point(size = 2.5, alpha = 0.9) + 
  geom_hline(yintercept = 0.5, linetype = 2, color = "gray30") +
  geom_vline(xintercept = 1200, linetype = 2, color = "gray30") +
  scale_color_manual(name = "Clade", values = sps.color) +
  ylab("Tandem repeat as % of non NTD") + xlab("Length of non-NTD part") +
  xlim(0,4500) + ylim(0,1) +
  theme_cowplot() + panel_border(color = "black")
ggsave("output/img/20210627-tandem-repeat-proportion-in-nonNTD.png", width = 6, height = 4.5)
```
...
tr.length %>% 
  ...
  geom_point(size = 2.5, alpha = 0.7) + 
  annotate(geom = "text", x = 2000, y = 1000, label = lm_eq(lm.obj), parse = TRUE) +
...
ST.comb <- bind_rows(S = S.freq, T = T.freq, .id = "Var") %>% 
  mutate(Var = ordered(Var, levels = c("S","T")))
p.st <- ggplot(ST.comb, aes(x = id, y = pos)) + geom_tile(aes(fill = freq), size = 2) + 
  ...
  #scale_fill_viridis_b(n.breaks = 6, begin = 0.1) +
  theme(axis.text.y = element_text(size = 4), axis.title.y = element_blank(),
        axis.line.y = element_blank(), axis.ticks.y = element_blank(),
        axis.line.x = element_blank(),# axis.ticks.x = element_blank(),
...
Plot the proteome-wide distribution:
...
bgST.freq %>% 
  ...
  theme_cowplot()
ggsave("output/img/20210701-ST-freq-proteome-wide-distribution.png", width = 6, height = 4)
```
```{r}
bgST.freq %>% 
...
```{r tango_sequences, fig.width=6, fig.height=7.5}
...
# plot
p1 <- ggplot(filter(feature, type == "entire protein"), aes(x = name, y = start)) + 
  geom_segment(aes(xend = name, yend = end), size = 2, color = "grey")
p2 <- geom_segment(data = filter(feature, type == "Hyphal_reg_CWP"), aes(x = name, y = start, xend = name, yend = end), size = 2, color = "#1f78b4")
...
p4 <- p1 + p2 + p3 + coord_flip() + theme_classic() + 
  scale_color_viridis_c(limits = c(0,101), breaks = seq(25,100,25)) +
  theme(axis.text.y = element_text(size = 4), axis.title.y = element_blank(),
        axis.line.y = element_blank(), axis.ticks.y = element_blank(),
        axis.line.x = element_blank(), axis.ticks.x = element_blank(),
        plot.title = element_text(hjust = 0.5),
...
_C. auris_ Hil1, Hil2 and Hil3 and their MDR orthologs are exceptional in TANGO hits number and distribution. They are on average longer than the rest of the homologs
...
tango1.sum %>%
  ...
  summarize(n = n(), mLen = median(length), mTANGO = median(n.all), mTANGO30 = median(n.gt30))
```
...
p1 <- motif.per.seq %>% 
  ...
  geom_point(aes(shape = reg.spaced, size = reg.spaced), alpha = 0.7, position = position_jitter(0.3)) +
...
p2 <- motif.per.seq %>% 
  ...
  #ggplot(aes(x = species, y = mad.intv)) + 
  geom_point(aes(pch = reg.spaced), alpha = 0.7, position = position_jitter(0.3), size = 2) +
  scale_shape_manual(name = "Regularly spaced", values = c(21, 19)) +
  scale_y_continuous(trans = "sqrt", 
                     breaks = c(0,1,10,100), minor_breaks = NULL) +
  ylab("Variation of spacing (MAD)") + coord_flip()

prow <- plot_grid(#p0 + theme_classic(12) + theme(axis.title.y = element_blank()), 
                  p1 + theme_gray(12) + 
          ...
          p2 + theme_gray(12) + theme(axis.text.y = element_blank(), axis.title.y = element_blank(),
                     legend.position = "none"),
          ncol = 2)

plot_grid(legend, prow, ncol = 1, rel_heights = c(.1,1))

ggsave("output/img/20210630-tango-hits-number-and-interval.png", width =5, height = 5)
```
...
freq.plot %>% 
  ...
  scale_y_continuous(name = "Proportion", trans = "reverse") + 
  theme_cowplot() + panel_border() +
  theme(legend.position = c(0.75, 0.1),
        strip.text.x = element_text(face = 3))

ggsave("output/img/20210303-compare-homologs-chromosomal-locations-to-bg.png", width = 8, height = 5)
```
