...
feature <- feature %>% 
  mutate(name = ordered(name, levels = rev(genetreeOrder)),
         type = ordered(type, levels = c("entire protein", "Hyphal_reg_CWP", "SignalP", "GPI-anchor", "Tandem Repeats")))
feature.colors <- c(Hyphal_reg_CWP = "#3d85c6", SignalP = "#cc0000", "GPI-anchor" = "#6a3d9a", "Tandem Repeats" = "#af8400bb")
```
...
ggsave("output/img/20210625-domain-tandem-repeats.png", width = 6, height = 7)
```
...
tr.length %>% 
  ...
  #scale_color_manual(name = "Clade", values = sps.color) +
  ylab("Tandem repeat content") + xlab("Protein length") +
  theme_cowplot() + panel_border(color = "black")
ggsave("output/img/20211120-tandem-repeat-proportion.png", width = 5, height = 4)
```
```{r}
