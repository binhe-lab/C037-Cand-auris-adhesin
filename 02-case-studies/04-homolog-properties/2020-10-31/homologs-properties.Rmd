...
p
```
**Fig. 1 Phylogenetic relationship of the species analyzed in this study.** This species tree is mostly based on the Maximum likelihood phylogeny from Muñoz _et al._ 2018 (PMID: 30559369), which used 1570 core genes. The relationships among the Saccharomycetaceae family is based on Gabaldón _et al._ 2016 (PMID: 27493146). The placement of _Debaryomyces hansenii_ differs between the above two publications. We based ours on the first publication. Note that in the latter publication as in Shen _et al._ 2018 (PMID: 30415838), which also places _D. hansenii_ closer to the Candida genus than to the Clavispora genus, the support for the part of the tree involving _D. hansenii_ is relatively low suggesting uncertainty in resolving the relationship. We chose to place _D. hansenii_ closer to the Clavispora genus because this is more congruent with the gene tree for the Hil family proteins we inferred below.

```{r}
p.tree <- ggtree(sps.tree, ladderize = FALSE) + xlim(0,12) + scale_y_reverse() +
  geom_tiplab(size = 4, align = TRUE, linesize = .5, face = 4) +
...
plot_grid(p.tree, p, align = "h", ncol = 2, rel_widths = c(1,2), scale = c(1.08,1))
```
_Discussion_
