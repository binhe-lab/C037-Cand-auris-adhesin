...
# author: Bin He
# date: 2022-05-03
# title: blast NTD of XP_028889033 against nakaseomyces species
# use: sh blast-nakaseomyces-gryc.sh
#########################
...
base=./Nakaseomyces-Hil-homologs-blast
db=Nakaseomyces-GRYC

# make blast database
cat 20220503-gryc-nakaseomyces-protein.faa.gz | gunzip -c | makeblastdb -in - -parse_seqids -dbtype prot -title $db -out ../blastdb/$db

# blastp (assumes the blast+ package has been installed locally)
blastp -db ../blastdb/$db -query $in -evalue 1e-5 -seg "no" -max_hsps 2 -num_threads 4 -outfmt 11 -out $base.asn
