...
sps.final <- unique(dedup.merge$species)
# convert the species names to the Shen 2018 name
sps.final.shen <- deframe(spsNameSyn)[sps.final]
cat("The following species are not present in the Shen 2018 list and need to be added later.")
setdiff(sps.final, spsNameSyn$ncbi.name)
```

```{r}
sps.final.shen %>% na.omit() %>% gsub(" ", "_", .) %>%
...
gene2sps.map <- dedup.merge %>% 
  select(name, species) %>% 
  mutate(treeName = ifelse(species %in% names(sps.final.shen),  sps.final.shen[species], species),
...
```{r}
write_tsv(sps.summary, file = "data/20220518-expanded-blast-species-info.tsv")
```
