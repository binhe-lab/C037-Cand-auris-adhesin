...
Merge the three datasets and export the fasta sequences and blast results.
```{r merge}
...
```{r}
# merge blast hit table
refseq.stat <-  refseq1 %>% 
  select(id = accession, species, strain, e_value = e.value, query_cover = query.cover, perc_ident = perc.ident, length)
fdb.stat <- fdb.filtered %>% 
  filter(gene_id %in% fdb.list) %>% 
  mutate(organism = gsub("strain ", "", organism),
         species = word(organism, 1, 2),
         strain = word(organism, 3, -1),
         query_cover = NA, perc_ident = NA) %>% 
  select(id = gene_id, species, strain, e_value, query_cover, perc_ident, length)
gryc.stat <- gryc %>% 
  filter(ori_id %in% gryc.list) %>% 
  mutate(organism = species,
         species = word(organism, 1, 2),
         strain = word(organism, 3, -1)) %>% 
  select(id, species, strain, e_value = min_e_value, query_cover, perc_ident = max_ident, length)
combined.blast <- bind_rows(list(refseq = refseq.stat, FungiDB = fdb.stat, GRYC = gryc.stat), .id = "source")
write_tsv(combined.blast, "output/XP_028889033_homologs_combine_blast_stat.tsv")

# also export the rejected list from refseq
refseq.rejected <- refseq.N360 %>% 
  filter(length < 500, sci.name %in% c("Candida parapsilosis", "Candida tropicalis MYA-3404", "Clavispora lusitaniae ATCC 42720",
                                       "Lodderomyces elongisporus NRRL YB-4239", "Scheffersomyces stipitis CBS 6054")) %>% 
  select(id = accession, species = sci.name, e_value = e.value, query_cover = query.cover, perc_ident = perc.ident, length)
write_tsv(refseq.rejected, "output/XP_028889033_homologs_length_filtered_blast_stat.tsv")
```
...
search_chr_loc <- function(gene){
  ...
  # 1: seq_name; 7: refseq_acc; 9: length
  final <- tibble(gene_id = gene, assembly = gInfo[3], chrloc = chr[1], chraccver = gInfo[7], 
...
tmp2 <- tmp2 %>% 
  rename(chr_name = chrloc) %>% 
  mutate(chrstart = as.integer(chrstart), chrstop = as.integer(chrstop), 
         chrL = as.integer(chrL), relLoc = round(chrstart/chrL, 2))
```

> The B9J08 sequence is the missing homolog from B11221. As shown in the README.md of this blast folder, I found that the "missing homolog" is actually present, but was broken into two fragments due to incomplete assembly. The C-terminus of the B9J08 sequence aligns to a fragment NW_021640165.1, which corresponds to scaffold00001. Its length is 4,303,397 bp, which is nearly the same as the reported chromosome 1 length from Muñoz _et al_. 2021 (Fig. 1). I therefore believe that this homolog (Hil4) is located at the 3' tip of chromosome 1 of B11221.
