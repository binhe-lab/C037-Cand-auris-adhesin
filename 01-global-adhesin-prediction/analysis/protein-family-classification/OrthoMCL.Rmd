...
frv.mcl5.sum <- frv.mcl5.sum %>% 
  left_join(select(frv.mcl5.og, group, domains, keywords, EC.num),
            by = c("group_id" = "group"))
...
```{r gather_gene_info, cache=TRUE, echo=FALSE}
...
```{r examine_og5_132045}
# get the protein IDs associated with the orthogroup
members <- frv.mapped5 %>% 
  filter(orthomcl_group_id == "OG5_132045") %>% 
  pull(protein_id)
...
```{r extract_ccd_info}
get_ccd <- function(pid){
  hit <- entrez_link(dbfrom = "protein", id = pid, db = "ccd")
