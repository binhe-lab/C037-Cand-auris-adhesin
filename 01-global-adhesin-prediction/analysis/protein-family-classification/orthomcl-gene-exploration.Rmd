...
subset <- c(fungalRV_0 = "frv_low",
            fungalRV_0.511 = "frv_high",
            FaaPred = "faapred",
            Both = "both")
# select the subset
selectInput(inputId = "sub", label = "Please select the criteria to apply", choices = c("fungalRV_0","fungalRV_0.511","FaaPred","Both"))

renderTable({
  frv.mcl5.sum[[subset[input$sub]]] %>% 
    filter(total > 2) %>% 
    select(-total) %>% 
    paged_table()# %>% 
    #kable() %>% 
