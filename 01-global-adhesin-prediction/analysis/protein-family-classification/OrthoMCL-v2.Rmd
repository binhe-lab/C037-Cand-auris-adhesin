...
frv.mapped.rest <- frv.mapped %>% 
  ...
  select(Species, Strain, protein_id, orthomcl_group_id, frv_score = Score, fungalRV, faapred)
...
frv.mapped.scer <- frv.scer %>% 
  left_join(select(scer.mapped, orthomcl_group_id = orthogroup_id, entrez_id), by = c("entrez_id"="entrez_id")) %>% 
  select(Species, Strain, protein_id = ID, orthomcl_group_id, frv_score = Score, fungalRV, faapred)
