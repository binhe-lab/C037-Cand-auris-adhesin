...
- Note that due to the 25 sequence limit of FaaPred, we didn't submit all sequences to it. Instead, we took the subset that got a FungalRV score greater than zero and used that as the input for FaaPred. The authors of the FungalRV tool recommended 0.511 as the cutoff. Here I'm using **`r frv.th`** as the cutoff, which is what the "FungalRV" column is based on.
...
frv.cmbd %>% 
  ...
  right_join(frv.stat) %>% 
  select(Species, Strain, mapped, self_group,Submitted)
```

### Reduce the three _C. auris_ strains to a single superset
...
Let's first examine the grouping of the _C. auris_ proteins, including the OrthoMCL groups and the MCL clustering result for those not mapped to the existing groups.
```{r examine_C_auris_proteins}
...
example_set <- frv.cmbd %>% filter(group_id == "OG5_132045") %>% pull(protein_id)
...
### Examine orthogroup membership of predicted adhesins

This analysis will be done on three different subsets of the data.
