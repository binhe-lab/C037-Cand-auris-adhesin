...
motif.summary[setdiff(match.jan, match0),] %>% filter(round(medScore,0) >= 30) %>% arrange(desc(n)) %>% select(seq, n, medScore, IVT, strain)
...
motif.per.seq <- motif.per.seq %>% 
  right_join(select(seqInfo, species_id, species_gr, name), by = "name") %>% 
