...
```{r}
seqInfo %>% 
  mutate(ST.protein = round(ST.prot, 2), ST.prot = NULL) %>% 
  select(-pfam_domains, pfam_domains) %>% 
  write_tsv(file = "output/Rmd-out/seqInfo_table.txt.gz")
```
...
filter3 <- filter2 %>%
  mutate(`S/T protein` = ST.prot > 0.18, ST.prot = round(ST.prot, 2)) %>% 
...
candidates %>% 
  ...
  write_tsv(file = "output/Rmd-out/20210722-candidate-leaders-seqinfo.tsv")
```
...
**Update 2021-07-20 [HB]** I decide to remove the clusters whose leader protein length is less than 200 a.a. This helps to shorten the list, making the plot more readable.

```{r}
# get the ids to plot
id.to.plot <- candidates %>% filter(is.leader, length >= 200) %>% pull(name)
...
feature.col <- c(brewer.pal(12, "Paired")[c(9,2,4,3,5,7,11,8)], brewer.pal(8, "Dark2")[c(2,8,3,4,6)], "skyblue")
