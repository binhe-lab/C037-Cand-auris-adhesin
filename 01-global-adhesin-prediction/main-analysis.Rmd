...
1. In the table above, rows 1, 3 and 4, along with XP_028893057, which is not in the candidates list because it misses the C-terminal GPI-anchor sequence (this is a partial mRNA missing the 3' portion), are the only ones containing the "GVVIVT" sequence (see README.md in this folder). They all have the GLEYA domain in the N-terminus, and had very similar central domain repeat units as the Hil1/2 protein.
1. Rows 2, 5 and 6 all have the Candida_ALS_N domain, and they are the ALS family members.
1. Examining the N-terminal Pfam domains among the rest candidate revealed Flo11, Asp, PLA2_B, CFEM, Glyco_hydro_72, SRP1_tIP1 and KRE1. Many of these domains have connections to adhesion.
...
**Update 2021-07-20 [HB]** I decide to remove the clusters whose leader protein length is less than 100 a.a. This helps to shorten the list, making the plot more readable, and the four proteins removed are spectaculously uninteresting (no pfam domain, no tandem repeats, etc.)

```{r}
# get the ids to plot
id.to.plot <- candidates %>% filter(is.leader, length >= 100) %>% pull(name)
...
df.whole <- candidates %>% 
  ...
  mutate(start = 1, end = length, feature = "whole protein",
         name = ordered(name, levels = id.to.plot)) %>% 
  select(name, feature, start, end, group)

df.repeats <- repeats %>% 
  mutate(feature = paste("TR", type, "-")) %>% 
  select(name = identifier, feature, start, end, period, type, copyN, consensus_nogap, consensus_error) %>% 
  filter(name %in% id.to.plot)

df.pfam <- hmmscan_pfam %>% 
  filter(!outcompeted,  # remove lower-scored overlapping domains
         !hmm_name %in% c("Hyr1", "Candida_ALS", "Flocculin_t3")) %>% # remove two repeat domains
  mutate(name = str_sub(seq_id, 2), # remove > from seq.id
         feature = paste0(hmm_name, " (", gsub("\\.[0-9]", "", hmm_acc), ")")) %>% # feature name
  filter(name %in% id.to.plot) %>% 
  select(name, feature, start = envelope_start, end = envelope_end)
...
df.gpi <- pred.gpi %>% 
  ...
  select(name, feature, start = cleavePos, end = length)
...
p0 <- ggplot(df.whole, aes(x = name, y = start, xend = name, yend = end)) + 
  ...
  geom_segment(color = "grey", size = 2)
# tandem repeats
p1 <- geom_segment(data = df.repeats, aes(size = "Tandem Repeats"), color = "orange4", alpha = 0.7)

# pfam
require(RColorBrewer)
feature.col <- colorRampPalette(brewer.pal(12, "Paired")[-12])(length(unique(df.pfam$feature))) # define the colors
...
# tango
p5 <- geom_segment(data = df.tango, aes(y = start-1,  yend = end + 1, alpha = median),
                   position = position_nudge(x = -0.4, y = 0), color = "darkred", size = 1)
...
p0 + p1 + p2 + p3 + p4 + p5 + coord_flip() + 
  ...
  scale_size_manual(
    ...
    guide=guide_legend(override.aes = list(colour=c("#6a3d9a", "#e31a1c", "orange4")))
  ) +
  guides(alpha = "none") +
...
ggsave("output/figure/20210720-cwp-candidates-domain-schematic.png", width = 5, height = 6.5)
```
