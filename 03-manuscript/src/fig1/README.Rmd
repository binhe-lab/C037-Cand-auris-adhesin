...
p1 <- dat %>% filter(feature == "ST_freq") %>% 
  ...
  xlab("Amino acid position") + ylab("Ser/Thr frequency") + ylim(0,0.6) + xlim(1,3100) +
  theme(legend.position = c(0.9, 0.8))

p2 <- dat %>% filter(feature == "TANGO") %>% 
  ggplot(aes(x = pos, y = value)) + geom_line(size = 0.3) + ylab("TANGO Score") +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank()) + xlim(1,3100) +
  scale_y_continuous(breaks = c(0,50,100))
  
# domain features
domain <- read_tsv("protein-domain-features.txt")
domain$feature <- factor(domain$feature, levels = c("entire protein", "SignalP", "Hyphal_reg_CWP",
                                                    "Hyr1", "GPI"))
p3 <- ggplot(domain, aes(xmin = start, ymin = 0)) + 
  ...
  ylab("") + xlab("") + xlim(1,3100) + ylim(-0.5,1) + labs(fill = "Domain") +
  theme_classic() + theme(legend.position = "top") +
  theme(axis.text = element_blank(), axis.ticks = element_blank(), axis.line = element_blank())

# assembling
plot_grid(p3, p2, p1, nrow = 3, align = "v", rel_heights = c(2,2,4))
